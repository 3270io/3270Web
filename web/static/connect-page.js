document.addEventListener("DOMContentLoaded", () => {
    ...
    const hostsDeleteConfirm = document.querySelector("[data-hosts-delete-confirm]");
    const hostsDeleteConfirmCancelEls = document.querySelectorAll("[data-hosts-delete-cancel]");
    const hostsDeleteConfirmAccept = document.querySelector("[data-hosts-delete-accept]");
    ...
    const noopFocusTrap = { activate() {}, deactivate() {} };
    const hostsFocusTrap =
        window.ThreeSeventyWeb && window.ThreeSeventyWeb.createFocusTrap && hostsModal
            ? window.ThreeSeventyWeb.createFocusTrap(hostsModal)
            : noopFocusTrap;
    const sampleFocusTrap =
        window.ThreeSeventyWeb && window.ThreeSeventyWeb.createFocusTrap && sampleModal
            ? window.ThreeSeventyWeb.createFocusTrap(sampleModal)
            : noopFocusTrap;
    ...
    if (hostsConnectButton) {
        hostsConnectButton.addEventListener("click", () => {
            if (hostsConnectButton.disabled) {
                return;
            }
            const host = applySelectedHost();
            ...
            upsertSavedHost(savedHostNameInput ? savedHostNameInput.value : "", host);
            closeHostsModal();
            hostsConnectButton.disabled = true;
            setLoading();
            hostInput.form.submit();
        });
    }

    const openHostsDeleteConfirm = () => {
        if (!hostsDeleteConfirm) {
            return;
        }
        hostsDeleteConfirm.hidden = false;
        if (hostsDeleteConfirmAccept) {
            hostsDeleteConfirmAccept.focus();
        }
    };

    const closeHostsDeleteConfirm = () => {
        if (!hostsDeleteConfirm) {
            return;
        }
        hostsDeleteConfirm.hidden = true;
        if (hostsDeleteButton) {
            hostsDeleteButton.focus();
        }
    };
    ...
    if (hostsDeleteButton) {
        hostsDeleteButton.addEventListener("click", () => {
            ...
            openHostsDeleteConfirm();
        });
    }

    hostsDeleteConfirmCancelEls.forEach((el) => {
        el.addEventListener("click", closeHostsDeleteConfirm);
    });

    if (hostsDeleteConfirmAccept) {
        hostsDeleteConfirmAccept.addEventListener("click", () => {
            const host = normalizeHost(savedHostValueInput ? savedHostValueInput.value : "");
            closeHostsDeleteConfirm();
            if (!host) {
                setHostsStatus("Select a host first.");
                return;
            }
    ...
    if (sampleStartButton && hostInput && appSelect && portSelect) {
        sampleStartButton.addEventListener("click", () => {
            ...
            hideSampleModal();
            if (hostInput.form) {
                sampleStartButton.disabled = true;
                setLoading();
                hostInput.form.submit();
            }
        });
    ...
    document.addEventListener("keydown", (event) => {
        ...
        if (hostsDeleteConfirm && !hostsDeleteConfirm.hidden) {
            closeHostsDeleteConfirm();
            return;
        }
        if (sampleModal && !sampleModal.hidden) {
            hideSampleModal();
        }
        if (hostsModal && !hostsModal.hidden) {
            closeHostsModal();
