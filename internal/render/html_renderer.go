...
func (r *HtmlRenderer) Render(s *host.Screen, actionURL, id string) string {
	...
	sb.WriteString(fmt.Sprintf(`<form id="%s" action="%s" method="post" class="h3270-form">`, formName, actionURL))
	sb.WriteString("\n")

	if s.IsFormatted {
		r.renderFormatted(s, id, &sb)
	} else {
		// TODO: Unformatted support
		sb.WriteString("<p>Unformatted screen not fully supported yet.</p>")
	}
