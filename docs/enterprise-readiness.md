...
!!! success "Phase 1 is shipped; Phase 2 is under way"
    ...
    3270 field semantics. Four Phase 2 items have followed: concurrent
    session tabs, hotspots, find on screen, and screen history.
...
Four of the seven Phase 2 items are done:
...
**Still open in Phase 2:** server-side connection profiles, IND$FILE, and
the keyboard remapping UI.
...
blast radius came from assuming the session cookie had to change meaning.
Leaving it as "the active session" and adding a roster cookie beside it
reduced the change to one new file, four routes, and a tab bar — no
existing handler was modified. The refactor was avoidable, and worth
checking for before committing to one.
